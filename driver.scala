...
ONLY CHANGE F2015B4A70551P to Your ID
*/
object Driver{
  ...
  def main(args:Array[String]){
    ...
    //Dot product
    val res1=F2015B4A70551P.dotProduct(kernel1,kernel2)

    // println(prod)
    //convolution 
    val res2=F2015B4A70551P.convolute(image,kernel1,imagesize,kernel1size) 

    // println(result)

    //activation 
    val res3=F2015B4A70551P.activationLayer((x:Double)=>if(x>150) x else 0,image) 

    // println(result)

    //single pooling

    val res4=F2015B4A70551P.singlePooling(max,poolmat,size)
    //pooling

    val res5=F2015B4A70551P.poolingLayer(max,image,size) // max is a function

    //Normalize

    val res6=F2015B4A70551P.normalise(nmat) 

    //mixed layer

    val res7=F2015B4A70551P.mixedLayer(image,kernel1,imagesize,kernel1size,(x:Double)=>x,max,size) // max is a function
...
//  COMMENT NEXT LINE WHEN YOU UNCOMMENT THIS
	val res8=time{F2015B4A70551P.assembly(image,imagesize,w1,w2,b,kernel1,kernel1size,kernel2,kernel2size,kernel3,kernel3size,size)}
    //val res8=F2015B4A70551P.assembly(image,imagesize,w1,w2,b,kernel1,kernel1size,kernel2,kernel2size,kernel3,kernel3size,size)
